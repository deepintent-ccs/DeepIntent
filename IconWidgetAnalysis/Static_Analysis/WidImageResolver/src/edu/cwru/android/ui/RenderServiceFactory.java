...
public class RenderServiceFactory {
	...
	private FrameworkResources supportResources;
	private Map<ResourceType, Map<String, ResourceValue>> res;

	public Map<ResourceType, Map<String, ResourceValue>> getResTable() {
		return res;
	};
	...
	/**
	 * Creates a config. This must be a valid config like a device would return.
	 ...
	 * @return
	 */
	public static FolderConfiguration createConfig(int size1, int size2, ScreenSize screenSize, ScreenRatio screenRatio,
			ScreenOrientation orientation, Density density, TouchScreen touchScreen, KeyboardState keyboardState,
			Keyboard keyboard, NavigationState navigationState, Navigation navigation, int apiLevel) {
	...
	/**
	 * Returns a {@link ResourceResolver} for a given config and project
	 * resource.
	 *
	 * @param config
	 ...
	 * @return
	 */
	public ResourceResolver createResourceResolver(FolderConfiguration config, ResourceRepository projectResources,
			String themeName, boolean isProjectTheme) {

		Map<ResourceType, Map<String, ResourceValue>> configedProjectRes = supportResources
				.getConfiguredResources(config);
		configedProjectRes.putAll(projectResources.getConfiguredResources(config));

		Map<ResourceType, Map<String, ResourceValue>> configedFrameworkRes = mResources.getConfiguredResources(config);
		// configedFrameworkRes.putAll(supportResources.getConfiguredResources(config));
	 ...
	 * @param myLayoutlibCallBack
	 * @return
	 */
	public RenderService createService(ResourceResolver resources, FolderConfiguration config,
			IProjectCallback projectCallback, MyLayoutlibCallBack myLayoutlibCallBack) {
	 ...
	 * {@link #createService(ResourceResolver, FolderConfiguration, IProjectCallback)}
	 * since the {@link ResourceResolver} object is not cached by the caller.
	 *
	 * @param projectResources
	 * @param themeName
	 * @param isProjectTheme
	...
	// IProjectCallback projectCallback, LayoutlibCallback layoutcallback) {
	// ResourceResolver resources = createResourceResolver(
	// config, projectResources, themeName, isProjectTheme);
	//
	// RenderService renderService = new RenderService(
	// mLibrary, resources, config, projectCallback,layoutcallback);
	//
	// return renderService;
	// }
	...
	private boolean loadLibrary(File platformFolder) {
		...
		mLibrary = LayoutLibrary.load(layoutLibJar.getAbsolutePath(), log, "LayoutLibRenderer");
		...
		File supportResFolder = new File("support/res");
		if (supportResFolder.isDirectory() == false) {
			throw new IllegalArgumentException("support/res folder does not exist.");
		}
		supportResources = loadResources(supportResFolder, log);

		// get all the attr values.
		HashMap<String, Map<String, Integer>> enumMap = new HashMap<String, Map<String, Integer>>();

		FolderConfiguration config = new FolderConfiguration();
		res = mResources.getConfiguredResources(config);
		// res.putAll(supportResources.getConfiguredResources(new
		// FolderConfiguration()));
		...
		addResourceItems(enumMap, styleableItems);
		addResourceItems(enumMap, res.get(ResourceType.BOOL));
		addResourceItems(enumMap, res.get(ResourceType.COLOR));
		addResourceItems(enumMap, res.get(ResourceType.DRAWABLE));
		addResourceItems(enumMap, res.get(ResourceType.INTEGER));
		addResourceItems(enumMap, res.get(ResourceType.STRING));
		addResourceItems(enumMap, res.get(ResourceType.ID));
		addResourceItems(enumMap, res.get(ResourceType.LAYOUT));
		addResourceItems(enumMap, res.get(ResourceType.DIMEN));
		addResourceItems(enumMap, res.get(ResourceType.STYLE));
		addResourceItems(enumMap, res.get(ResourceType.STYLEABLE));
	...
	private void addResourceItems(HashMap<String, Map<String, Integer>> enumMap, Map<String, ResourceValue> boolItems) {
		for (ResourceValue value : boolItems.values()) {
