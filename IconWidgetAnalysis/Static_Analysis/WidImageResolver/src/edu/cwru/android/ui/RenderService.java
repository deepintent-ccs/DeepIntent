...
public class RenderService {
	...
	private MyLayoutlibCallBack mLayoutlibCallback;

	/** Use the {@link RenderServiceFactory#create} factory instead 
	 * @param myLayoutlibCallBack */
	RenderService(LayoutLibrary layoutLibrary, ResourceResolver resources, FolderConfiguration config,
		...
		mLayoutlibCallback = myLayoutlibCallBack;
	}

	/**
	 * Sets the {@link LayoutLog} to be used during rendering. If none is specified, a silent logger will be used.
	 *
	 * @param logger
	 *            the log to be used
	...
	/**
	 * Sets the overriding background color to be used, if any. The color should be a bitmask of AARRGGBB. The default
	 * is null.
	 *
	 * @param overrideBgColor
	 *            the overriding background color to be used in the rendering, in the form of a AARRGGBB bitmask, or
	 *            null to use no custom background.
	 * @return this (such that chains of setters can be stringed together)
	 */
	...
	/**
	 * Sets whether the rendering should include decorations such as a system bar, an application bar etc depending on
	 * the SDK target and theme. The default is true.
	 *
	 ...
	 */
	public RenderSession createRenderSession(String layoutName) throws FileNotFoundException, XmlPullParserException {
		finishConfiguration();
		...
		HardwareConfig hwConfig = new HardwareConfig(mWidth, mHeight, mConfig.getDensityQualifier().getValue(), mXdpi,
				mYdpi, mConfig.getScreenSizeQualifier().getValue(), mConfig.getScreenOrientationQualifier().getValue(),ScreenRound.ROUND,
				true);
//		hwConfig = new HardwareConfig(screenWidth, screenHeight, density, xdpi, ydpi, screenSize, orientation, screenRoundness, softwareButtons)
		SessionParams params = new SessionParams(parser, mRenderingMode, this, hwConfig, mResourceResolver,
				mLayoutlibCallback, mMinSdkVersion, mTargetSdkVersion, mLogger);
		
		
//		SessionParams(com.android.ide.common.rendering.api.ILayoutPullParser layoutDescription, 
		//com.android.ide.common.rendering.api.SessionParams.RenderingMode renderingMode, 
		//java.lang.Object projectKey, com.android.ide.common.rendering.api.HardwareConfig hardwareConfig, 
		//com.android.ide.common.rendering.api.RenderResources renderResources, 
		//com.android.ide.common.rendering.api.LayoutlibCallback layoutlibCallback, int minSdkVersion, int targetSdkVersion, 
	...
	/**
	 * Renders the given resource value (which should refer to a drawable) and returns it as an image
	 *
	...
	public BufferedImage renderDrawable(ResourceValue drawableResourceValue) {
		...
		figureSomeValuesOut();
		HardwareConfig hwConfig = new HardwareConfig(mWidth, mHeight, mConfig.getDensityQualifier().getValue(), mXdpi,
				mYdpi, mConfig.getScreenSizeQualifier().getValue(), mConfig.getScreenOrientationQualifier().getValue(),
				ScreenRound.ROUND ,false);

		DrawableParams params = new DrawableParams(drawableResourceValue, this, hwConfig, mResourceResolver,
				mLayoutlibCallback, mMinSdkVersion, mTargetSdkVersion, mLogger);
